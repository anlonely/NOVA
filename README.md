...
1. 打开 [Releases](https://github.com/anlonely/NOVA/releases/latest) 下载最新 `NOVA-INTERP-macOS-*.zip`。
2. 解压后双击 `NOVA INTERP.app`。
3. 如果系统提示安全限制，按系统安全设置允许打开。

#### Windows

1. 打开 [Releases](https://github.com/anlonely/NOVA/releases/latest) 下载最新 `NOVA-INTERP-windows-x64-*.zip`。
2. 解压后双击 `NOVA-INTERP.exe`。

### 2. 在“设置”里填入 Key（先这样可以直接运行）

打开应用后，进入左侧抽屉 **引擎接入 / Engine Access**，填写：

- `APP ID`
- `Access Token`（推荐先只填这个）
- `Secret Key`（兼容签名场景；如不确定可先留空）
- `资源 ID`（默认 `volc.service_type.10053`）

### 3. 开通火山引擎并拿到 APP ID / Access Token / secret_key（3-5 分钟）

1. 打开豆包语音控制台：<https://console.volcengine.com/speech/app>（登录并实名认证）
2. 创建应用，选择接入区域并勾选能力：`大模型语音翻译`（必选）、`声音复刻`（如需声音克隆）。
3. 在应用详情复制：`APP ID`、`Access Token`、`secret_key`
4. 如需签名鉴权（可选），访问 <https://console.volcengine.com/iam/credential/access-keys>。
5. 先保持 `资源 ID` 默认 `volc.service_type.10053`，首次联通后再按控制台推荐配置。

### 4. 先试用再正式开通（有额度可先打通）

- 试用可用性与额度以控制台实际显示为准：<https://www.volcengine.com/docs/6561/163043?lang=zh>
- 计费与鉴权可见：
  - [鉴权方式（Token/Signature）](https://www.volcengine.com/docs/6561/107789?lang=zh)
  - [计费说明（豆包语音）](https://www.volcengine.com/docs/6561/1359370?lang=zh)
  - [音色计费与试用（音色转换）](https://www.volcengine.com/docs/6561/637628?lang=zh)
...
- 启动后如出现“未配置密钥”提示，请先在左侧抽屉 **引擎接入 / Engine Access** 填写 `APP ID` 和 `Access Token` 后保存。
- 保存后点击“连接/开始”（不同语言包文案可能不同）启动链路；仍无响应可尝试重启应用。
...
├── custom_dns.py                   # AST 定向 DNS 覆盖与解析缓存
├── updater.py                      # 更新检测与下载逻辑
