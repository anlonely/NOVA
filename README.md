...
- `main`：当前对外主线（**V2 主线**，已承载 native audio core 的主干能力）。
- `old_v1`：保留历史 `main` 版本，作为回退与对账基线。

本地可按以下命令快速切换：

```bash
git fetch --all
git checkout old_v1      # 需要查看旧主线时
git checkout main        # 切回 V2 主线时
```
...
- [原生音频核心说明](native_audio_core/README.md)
...
├── launch_legacy_tk.ps1            # 旧版 Tk UI 启动脚本
